...
def test_promote_to_production_retires_current_prod_atomically(spark, registry_db):
    ...
    events_30 = [e for e in store.history("uplift", "3.0.0") if e.status == Status.RETIRED]
    events_31 = [e for e in store.history("uplift", "3.1.0") if e.status == Status.PRODUCTION]
